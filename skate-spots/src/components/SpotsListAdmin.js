...
import "../styles/SpotsListAdmin.css";

export default function SpotsListAdmin() {
  ...
  return (
    <div className="SpotsListAdmin">
      <h2 className="h2-header">Spots List (Admin View)</h2>
          ...
          <tbody>
            {spots.map((spot) => (
              <tr key={spot.id}>
                ...
                <td>
                  <Link to={`/edit-spot/${spot.id}`} state={{ spotData: spot }}>
                    <button>Edit</button>
                  </Link>
                </td>
                <td>
